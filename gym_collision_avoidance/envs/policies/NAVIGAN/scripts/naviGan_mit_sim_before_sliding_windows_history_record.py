...
class NAVIGANPolicy(InternalPolicy):
    ...
    def find_next_action(self, obs, agents, i ):
        ...
        for a in range(self.n_agents):
            ...
            if (np.mod(timestep_now, timestep_history_interval )==0):  #dataset 0.4s per data, so only record 1 time every 4 data
                #
                self.agents_history[a, timestep_now//timestep_history_interval ] = [ agents[a].global_state_history[1] , agents[a].global_state_history[2] ]
        ...
        #but navigan take in [history_size,  number of agent,  size of xy tuple]
