...
def evaluate(
        args, obs_traj, obs_traj_rel, seq_start_end, goals, goals_rel,  obs_delta, attention_generator, discriminator, agent_index, plot_dir=None
    ):
    ade_outer, fde_outer = [], []
    total_traj = 0
    count = 1
    ...
    for path in paths:
        ...
        for time_ind in range(len_obs):
            ...
            for agent_ind in range(num_ped):
                ...
                temp.append([  time_ind, 0 ])
                
            observation_input.append( temp  )

        goal = [20, 0]
        goal = torch.from_numpy( np.array( goal )  )
        ...
        evaluate(    
            _args, obs_traj, obs_traj_rel, seq_start_end, goals, goals_rel, obs_delta, attention_generator, discriminator, agent_index,
